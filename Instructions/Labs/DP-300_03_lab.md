...
1. Azure SQL データベースのファイアウォールを構成する

2. Azure Active Directory で Azure SQL データベースへのアクセスを承認する

3. Azure SQL データベースの Advanced Threat Protection を有効化する

4. Azure SQL データベースのデータ分類の実装

5. データベース オブジェクトへのアクセスの管理

# シナリオ

データベース環境のセキュリティを確保するために、シニア データベース管理者として採用されました。これらのタスクは、Azure SQL データベースに焦点を当てます。 

 

## 演習 1: Azure SQL データベースのファイアウォールを構成する 
...
5. 「スタート」 メニューから 「Microsoft SQL Server ツール18 」 > 「SQL Server Management Studio」 に移動して、ラボ VM で SQL Server Management Studio を開きます。Azure SQL Database サーバーの名前を貼り付け、ラボ 2 で作成した資格情報でログインします。
	...
	![自動生成された携帯電話の説明のスクリーンショット](images/dp-3300-module-33-lab-05.png)
...
9. 「データベース設定」 画面で、Azure SQL Database のエディションを General Purpose に変更します。サービス目標を GP_Gen4_2 に変更し、「次へ」 をクリックします。 

 

	![自動生成された携帯電話の説明のスクリーンショット](images/dp-3300-module-33-lab-10.png)

 

10.  「**次へ**」 をクリックしてから、サマリ画面で 「**終了**」 をクリックします。インポートが完了すると、以下の結果が表示されます。
	![自動生成された携帯電話の説明のスクリーンショット](images/dp-3300-module-33-lab-11.png)
...
11. オブジェクト エクスプローラーで、AdventureWorks を右クリックし、「新しいクエリ」 をクリックします。 
	...
	![自動生成された携帯電話の説明のスクリーンショット](images/dp-3300-module-33-lab-12.png)

 

12. クエリ画面にテキストを貼り付け、「実行」 をクリックするか、F5 キーを押して、次の T-SQL クエリを実行します。
	...
	192.168.1.1. を手順 4 からのクライアント IP アドレスに置き換えます。次に、AdventureWorks データベースに含まれるユーザーを作成します。「新しいクエリ」 をクリックして、次の T-SQL を実行します。AdventureWorks データベースを使用していることを確認してください。下の強調表示されたボックスに 「マスター」 が表示されている場合は、プルダウンして、「AdventureWorks」 に切り替えることができます。
...
## タスク 2: Azure Active Directory で Azure SQL データベースへのアクセスを承認する
...
2. Azure portal で Azure SQL データベース サーバー dp300-lab-xxに移動し、Active Directory 管理者の横にある、「未構成」 をクリックします。
...
3. SQL Server Management Studio を開き、「接続」 > 「データベース エンジン」 をクリックします。サーバー名に自分のサーバーの名前を入力します。認証タイプをMFAで、Azure Active Directory Universal に変更します。
	...
	Azure Active Directory パスワードの入力を求められ、データベースにログインします。 
...
3. Azure SQL データベースのメイン ブレードの 「セキュリティ」 セクションに移動し、「Advanced Data Security」 をクリックします。Advanced Data Security 画面で、データ検出と分類をクリックします。
